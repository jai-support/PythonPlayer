...
def PvSelectDevice():
    ...
    if (
        lSelectedDI.GetType() == eb.PvDeviceInfoTypeUSB
        or lSelectedDI.GetType() == eb.PvDeviceInfoTypeU3V
    ):
    ...
    if isinstance(lSelectedDI, eb.PvDeviceInfoGEV):
        ...
        lResult = eb.PvDeviceGEV.SetIPConfiguration(
            lSelectedDI.GetMACAddress(),
            lNewIPAddress,
            lSelectedDI.GetSubnetMask().GetAscii(),
            lSelectedDI.GetDefaultGateway().GetAscii(),
        )
        if not lResult.IsOK():
    ...
    while not kb.is_stopping():
        ...
        print(
            f"The device {lNewIPAddress} was not located. Do you want to continue waiting? (y/n)"
        )
        print(">", end="")
        lAnswer = kb.getch()
        if lAnswer == "n":
